...
def signup():
    ...
    if st.button("Signup"):
        if not is_valid_email(email):
            ...
            st.error(
                "Incorrect SMTP server domain, please provide a valid gmail account.")
        else:
            ...
            if result.returncode != 0:
                ...
                try:
                    # to check if user exists and we are successfully able to retrieve their emails, only then we can log them in
        ...
        for idx, email in enumerate(emails, start=1):
            with st.expander(email["subject"] or email["body"][:20] + '...' if len(email["body"]) > 20 else email["body"]):
                ...
                if st.button(f"Delete Email {idx}", key=f"delete_email_{email['mail_id']}"):
                    delete_email(
                        st.session_state["user"], st.session_state["password"], email["mail_id"])
                    st.rerun()
...
def compose_email_page():
    ...
    if st.button("Send"):
        if email_content.strip() == "":
            ...
            try:
                send_email(
                    st.session_state["user"], recepient, subject, email_content, attachment)
